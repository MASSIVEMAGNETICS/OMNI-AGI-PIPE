...
def main():
    ...
    commands:
      ...
      connect [src_idx] [dst_idx] - connect nodes by index
    ...
    """)
                ...
                graph.load(path)
                node_map = list(graph.nodes.keys())
                ...
                node_map.append(nid)
                AGIUtils.log(f"Added node {nid} of type {ntype} (index: {len(node_map) - 1})")
            elif cmd.startswith("connect"):
                _, src_idx, dst_idx = cmd.split()
                src_id = node_map[int(src_idx)]
                dst_id = node_map[int(dst_idx)]
                graph.connect(src_id, dst_id)
                AGIUtils.log(f"Connected {src_id} -> {dst_id}")
