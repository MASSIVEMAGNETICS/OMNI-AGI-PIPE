...
class GQAFractalAttention(nn.Module):
    ...
    def forward(self, x: torch.FloatTensor, mask: Optional[torch.BoolTensor] = None) -> torch.FloatTensor:
        ...
        # Attention
        attn_scores = torch.einsum("blghd, blkhd -> blghk", q, k) * self.scale
        if mask is not None:
            # Mask needs to be broadcastable to (B, L, q_groups, heads_per_group, L)
            attn_scores = attn_scores.masked_fill(mask[:, :, None, None, :], -1e4)
        attn = F.softmax(attn_scores, dim=-1)
        out = torch.einsum("blghk, blkhd -> blghd", attn, v)
